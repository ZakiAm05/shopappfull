...
const ProductList = async ({
  ...
  return (
    <div className="mt-12 flex gap-x-8 gap-y16 justify-between flex-wrap">
      {res.items.map((product: products.Product) => (
        ...
        >
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold">{product.price?.price} DZD</span>
          </div>
              ...
              dangerouslySetInnerHTML={{
                __html: DOMPurify.sanitize(
                  product.additionalInfoSections?.find(
                    (section: any) => section.title === "Short desc"
                  )?.description || ""
                ),
              }}
            ></div>
